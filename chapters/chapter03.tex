...
En síntesis: el diferencial no es una funcionalidad aislada, es un cambio de enfoque completo, de un modelo de ``detectar y avisar'' (las tres soluciones relevadas) a uno de ``detectar y actuar'' (el proyecto), que cierra exactamente la brecha que la propia conclusión del Estado del Arte del proyecto identifica como la carencia principal del mercado actual: la etapa de respuesta ante incidentes.
...
\begin{itemize}
	\item \textbf{Business} (detección + respuesta automática configurable): USD 2 a 5 por \emph{endpoint}/mes. Para una PyME de referencia con 50-100 equipos, eso da un gasto total de aproximadamente USD 100 a 500 por mes (un monto que una PyME puede aprobar sin pasar por un comité de compras extenso).
	\item \textbf{Enterprise} (incluye reentrenamiento periódico configurable y soporte prioritario): USD 6 a 10 por \emph{endpoint}/mes. Sobre la misma base de 50-100 equipos, esto da un gasto total de entre USD 300 y 1.000 por mes.
\end{itemize}
...
Debido a esta distribución de responsabilidades, el proyecto no está planteado como una aplicación monolítica, sino como un sistema compuesto por varios componentes que trabajan de manera conjunta; por eso las tecnologías utilizadas en cada parte fueron seleccionadas teniendo en cuenta la función que cumple cada componente dentro de la arquitectura general. El proyecto está organizado como un \emph{monorepo} que contiene seis subsistemas principales: la API encargada del \emph{scoring} en tiempo real (\texttt{api/}), el agente PEP local (\texttt{agent/}), el \emph{dashboard} web (\texttt{frontend/}), el \emph{pipeline} de entrenamiento offline del modelo (\texttt{ml\_core/}), los \emph{scripts} de demostraciones, reproducción de eventos y validaciones (\texttt{scripts/}) y la documentación de arquitectura (\texttt{docs/}).
...
\begin{longtable}{p{3cm}p{11.6cm}}
	...
	\hline
	Uvicorn & Servidor ASGI de referencia para FastAPI; requerido porque FastAPI está construido sobre Starlette/ASGI, no sobre WSGI. \\
	\hline
	...
	\hline
	TensorFlow / Keras (LSTM Autoencoder) & Necesario porque IsolationForest trabaja sobre un vector agregado diario y no captura el orden de los eventos dentro de una sesión. El LSTM Autoencoder sí modela dependencias secuenciales (ventanas de 50 eventos consecutivos), que es exactamente el tipo de anomalía que un agregado diario diluye (por ejemplo, la secuencia atípica ``abrir carpeta protegida $\rightarrow$ comprimir $\rightarrow$ conectar a servicio externo'' en un orden y ritmo inusual, aunque el volumen total del día no sea anómalo). Frente a PyTorch: Keras da una API de alto nivel suficiente para un \emph{autoencoder} LSTM estándar sin escribir el \emph{loop} de entrenamiento a mano, reduciendo la superficie de error en el componente más costoso de mantener del \emph{pipeline}. \\
	\hline
...
\begin{longtable}{p{3cm}p{11.6cm}}
	...
	\hline
	\endhead
	requests & Cliente HTTP mínimo para \texttt{ScoringClient} (\texttt{POST /eventos}). Se prefirió sobre \texttt{httpx}/\texttt{aiohttp} porque el agente no necesita concurrencia asíncrona real (es un \emph{loop} de sondeo secuencial: \texttt{while True: poll → post → sleep}), y \texttt{requests} es la opción más liviana y estable para ese patrón síncrono. \\
	\hline
...
\begin{longtable}{p{3cm}p{11.6cm}}
	...
	\hline
	Oxlint & \emph{Linter} escrito en Rust, sensiblemente más rápido que ESLint tradicional; una elección razonable de costo/beneficio para un proyecto con tiempo de desarrollo limitado, aunque la configuración quedó en el \emph{default} de la plantilla de Vite, sin personalizar reglas propias del proyecto. \\
	\hline
\end{longtable}
...
\begin{longtable}{p{3cm}p{11.6cm}}
	...
	\hline
	imbalanced-learn & Declarada en \texttt{ml\_core/requirements.txt}, es coherente con que el \emph{dataset} de entrenamiento tiene una proporción de eventos anómalos extremadamente baja (253 sesiones etiquetadas sobre 7,6 millones de eventos en el \emph{dataset} legado LANL), un escenario típico donde se necesitan técnicas de balanceo de clases. \\
	\hline
